...
function empiezaConNueve(num) {
   ...
   // Tu código:
   if ((Math.floor(Math.floor(num)/Math.pow(10,Math.max(Math.floor(Math.log10(Math.abs(num))), 0))))===9){
      return true;
      } else {
      return false;}
}
...
function continueStatement(num) {
   ...
   for(i=1;i<11;i++){
       ...
       num = num +2;
       arrayAumentaDosb.push(num);    
          }
   }
   return arrayAumentaDosb;
   }
